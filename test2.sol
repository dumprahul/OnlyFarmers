...
contract DeFiStakingProtocol {
    ...
    function withdraw() external {
        ...
        if (stakingDuration < EPOCH_DURATION) {
            ...
            // Calculate rewards - APY * stakedAmount / 12 (for monthly distribution)
            reward = (staker.yieldPercentage * stakedAmount) / 1200;
    ...
    function getStakeInfo(address _address) external view returns (
        ...
        if (stakingDuration >= EPOCH_DURATION) {
            reward = (staker.yieldPercentage * staker.amount) / 1200;
