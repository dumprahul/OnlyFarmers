...
contract FarmBasedDeFiStakingProtocol {
    ...
    // Events
    event FarmCreated(uint256 indexed farmId, address indexed farmerAddress, uint256 farmAPY);
    event FarmUpdated(uint256 indexed farmId, uint256 farmHealth, uint256 yieldScore, uint256 farmAPY);
     ...
     */
    function createFarm(uint256 _farmHealth, uint256 _yieldScore, uint256 _farmAPY) external returns (uint256) {
        ...
        emit FarmCreated(newFarmId, msg.sender, _farmAPY);
     ...
     */
    function updateFarm(uint256 _farmId, uint256 _farmHealth, uint256 _yieldScore, uint256 _farmAPY) external {
        require(_farmId > 0 && _farmId <= farmCount, "Invalid farm ID");
        require(farms[_farmId].farmerAddress == msg.sender, "Only farm owner can update farm");
        require(_farmHealth > 0, "Farm health must be greater than 0");
        require(_yieldScore > 0, "Yield score must be greater than 0");
        require(_farmAPY > 0, "Farm APY must be greater than 0");
    ...
    function stake(uint256 _farmId) external payable {
        ...
        if (stakers[msg.sender].active) {
            require(stakers[msg.sender].farmId == _farmId, "Already staking in a different farm");
    ...
    function withdraw() external {
        ...
        if (stakingDuration < EPOCH_DURATION) {
            ...
            payable(farm.farmerAddress).transfer(penaltyHalf);
            ...
            // Send funds
            payable(farm.farmerAddress).transfer(farmerReward);
